...
export const seminovoSchema = z.object({
    modeloMotor: z.string(textMessage),
    quantidadeMotor: z.number(numberMessage).positive(numberMessage),
    ...
    combustivel: z.string(textMessage).max(50),
    propulsao: z.string(textMessage).max(50),
    moeda: z.string(textMessage).max(50), 
    preco: z.string(numberMessage),
    ...
    procedencia: z.string(textMessage).max(50,{message: "Número máximo de caracteres: 50"}),
    imagens: z.array(z.string({message: "Erro ao carregar imagens"}), {message: "Erro na estrura de dados das imagens"}).min(numeroMinimoImagens, {message: `Adicione pelo menos ${numeroMinimoImagens} imagens para seu barco.`}),
